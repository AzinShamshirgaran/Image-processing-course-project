...
length_interval = 0.1 * 48000;
% The number of intervals.
n_intervals = length / length_interval;
...
for i = 1:n_intervals
    ...
    x = [x cos(2*pi*freq * samples(((i-1)*(length_interval)+1:((i-1)*(length_interval)+length_interval))))];
end

% Play the sine waves.
sound(x, sample_rate);
% Save it.
audiowrite('two_sines_alt.wav', x, sample_rate);
...
% Save as png file.
imwrite(s_spectrum_log_scale,'two_sines_alt_spect.png');
